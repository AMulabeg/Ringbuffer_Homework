...
int main() {
  ...
  char *msg[5] = {"Hello World. Nice to meet you.", "This is a test message.",
                  "Another message.", "Yet another message.",
                  "The last message."};

  size_t msg_len[5] = {/* include the null terminator */
                       strlen(msg[0]) + 1, strlen(msg[1]) + 1,
                       strlen(msg[2]) + 1, strlen(msg[3]) + 1,
                       strlen(msg[4]) + 1};
