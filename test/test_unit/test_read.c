...
int main() {
  ...
  if (ringbuffer_read(ringbuffer_context, buffer, &buffer_len) !=
      RINGBUFFER_EMPTY) {
  ...
  if (ringbuffer_context->read != (uint8_t *)rbuf ||
      ringbuffer_context->read != ringbuffer_context->begin) {
    printf("Error: Test 2.1 failed. Write failed. Fix write first.\n");
    exit(1);
  }
  ...
  if (ringbuffer_read(ringbuffer_context, buffer_too_small, &buffer_len) !=
  ...
  if (ringbuffer_context->write !=
      (uint8_t *)rbuf + 2 * (msg_len + sizeof(size_t))) {
    printf("Error: Test 2.2 failed. Write failed. Fix write first.\n");
    exit(1);
  }
  ...
  memcpy(&read_len, ringbuffer_context->write - msg_len - sizeof(size_t),
         sizeof(size_t));
  if (read_len != msg_len) {
    printf("Error: Test 2.2 failed. Write failed. Fix write first.\n");
    exit(1);
  ...
  ringbuffer_context->read = (uint8_t *)rbuf + msg_len +
                             sizeof(size_t); // simulate that read pointer is
                                             // there where we wrote the message
  ringbuffer_context->write = (uint8_t *)
      rbuf; // advance write pointer to the beginning (simulate wrap around)
  ...
  ringbuffer_init(ringbuffer_context, rbuf, rbuf_size);
  ringbuffer_context->read =
      (uint8_t *)rbuf + 3 * (msg_len + sizeof(size_t)) -
      sizeof(size_t); // ensure that there is enough space to write the message
  ringbuffer_context->write = (uint8_t *)rbuf + 3 * (msg_len + sizeof(size_t)) -
                              sizeof(size_t) +
                              1; // this enduces a wrap around of the prefix
  ...
  if (ringbuffer_context->read !=
      (uint8_t *)rbuf + 3 * (msg_len + sizeof(size_t)) - sizeof(size_t)) {
    printf("Error: Test 3.2.1 failed. Write failed. Fix write first.\n");
    exit(1);
  }
  ...
  ringbuffer_context->read =
      (uint8_t *)rbuf + 3 * (msg_len + sizeof(size_t)) - sizeof(size_t) +
      1; // simulate that read pointer is there where we wrote the message
  ...
  ringbuffer_init(ringbuffer_context, rbuf, rbuf_size);
  ringbuffer_context->read =
      (uint8_t *)rbuf + rbuf_size - 2 * sizeof(size_t) -
      1; // ensure that there is enough space to write the message
  ringbuffer_context->write =
      (uint8_t *)rbuf + rbuf_size -
      2 * sizeof(size_t); // this enduces a wrap around of the message
  ...
  if (ringbuffer_context->read !=
      (uint8_t *)rbuf + rbuf_size - 2 * sizeof(size_t) - 1) {
    printf("Error: Test 3.2.2 failed. Write failed. Fix write first.\n");
    exit(1);
  }
  ...
  ringbuffer_context->read =
      (uint8_t *)rbuf + rbuf_size -
      2 * sizeof(size_t); // simulate that read pointer is there where we wrote
                          // the message
  memcpy(&read_len, ringbuffer_context->read, sizeof(size_t));
  if (read_len != msg_len) {
    printf("Error: Test 3.2.2 failed. Write failed. Fix write first.\n");
  ...
  ringbuffer_init(ringbuffer_context, rbuf, rbuf_size);
  ringbuffer_context->read +=
      2 * (msg_len + sizeof(size_t)) -
      2; // ensure that there is enough space to write the message
  ringbuffer_context->write += 2 * (msg_len + sizeof(size_t)) -
                               1; // this enduces a wrap around of the message
  ...
  if (ringbuffer_context->read !=
      (uint8_t *)rbuf + 2 * (msg_len + sizeof(size_t)) - 2) {
    printf("Error: Test 3.2.3 failed. Write failed. Fix write first.\n");
    exit(1);
  }
  ...
  ringbuffer_context->write +=
      2 *
      (msg_len + sizeof(size_t)); // this enduces a wrap around of the message
  ...
  if (ringbuffer_context->read !=
      (uint8_t *)rbuf + 2 * (msg_len + sizeof(size_t)) - 1) {
    printf("Error: Test 3.2.4 failed. Write failed. Fix write first.\n");
    exit(1);
  }
  ...
  ringbuffer_context->read =
      (uint8_t *)rbuf +
      2 * (msg_len + sizeof(size_t)); // simulate that read pointer is there
                                      // where we wrote the message
